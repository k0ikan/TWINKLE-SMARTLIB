    ...
    with col2:
        with st.container():
            ...
            with tab_login:
                ...
                # Google Login Button
                st.markdown("""
                <a href="https://accounts.google.com/oauth/authorize" target="_self">
                    <button class="google-btn" style="width: 100%; padding: 12px; border-radius: 8px; border: none; font-weight: 600; cursor: pointer;">
                        🔗 Masuk dengan Google
                    </button>
                </a>
                """, unsafe_allow_html=True)
                ...
                email = st.text_input("Email atau Username", placeholder="email@anda.com")
                ...
                with col_btn:
                    if st.button("MASUK", use_container_width=True):
                        if email and password:
                            ...
                            st.session_state['user_data']['nama'] = email.split('@')[0].title()
                ...
                with col_lupas:
                    st.caption("[Lupa password?](#)")
                
                st.markdown("---")
                st.caption("Belum punya akun? Daftar di atas ↑")
            ...
            with tab_daftar:
                ...
                email = st.text_input("Email", placeholder="email@anda.com")
                password = st.text_input ("Password", type="password", placeholder= "•••••••")
                konfirmasi = st.text_input("Konfirmasi Password", type="password", placeholder="••••••••")
                
                col_terms = st.checkbox("Saya setuju dengan Syarat & Ketentuan")
                
                if st.button("DAFTAR SEKARANG", use_container_width=True):
                    if nama and email and password and konfirmasi:
                        if password == konfirmasi:
                            if col_terms:
                                st.session_state['logged_in'] = True
                                st.session_state['user_data']['nama'] = nama
                                st.session_state['user_data']['email'] = email
                                st.session_state['user_data']['login_method'] = 'google'
                                st.success("Pendaftaran berhasil! Selamat datang!")
                                st.rerun()
                            else:
                                st.error("Setuju dengan Syarat & Ketentuan!")
                        else:
    ...
    if q:
        if cek_kuota():
            ...
            if hasil:
                ...
                """, unsafe_allow_html=True)
                if st.button(f"📚 Pinjam {hasil['judul']}"):
